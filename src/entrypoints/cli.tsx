...
// 先做一个小修正：
// 关掉 corepack 自动改配置的行为，免得它偷偷往 package.json 里塞 yarnpkg。
// eslint-disable-next-line custom-rules/no-top-level-side-effects
process.env.COREPACK_ENABLE_AUTO_PIN = '0';

// 如果程序跑在远程/容器环境里，就多给子进程一点内存，
// 免得任务一大时更容易因为内存不够出问题。
...
// 如果开了某个实验模式，就先把几项“精简模式”开关打开。
// 之所以写在这里，是为了让后面加载进来的代码一开始就看到这些设置。
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level            
...
async function main(): Promise<void> {
  // 把命令行后面跟着的参数取出来。
  // 下面基本都是靠它来判断“现在到底要做什么”。
  const args = process.argv.slice(2);

  // 如果用户只是想看版本号，就直接打印然后结束。
  // 这种情况很简单，没必要加载后面一大堆代码。
  ...
  // 从这里开始，就不是“只看版本号”这种最简单的情况了。
  // 所以顺手记录一下启动过程，方便以后排查哪里慢。
  const {
    profileCheckpoint
  ...
  // 这是一个内部调试用法：
  // 把程序最终要用到的 system prompt 打印出来，然后马上结束。
  // 你可以先把它理解成“把内置提示词原样导出来”。
  ...
  // 这一段是在处理和浏览器联动的几种特殊启动方式。
  // 只要命中其中一种，就直接跳去执行对应功能。
  ...
  // 这一段处理 remote-control / bridge 这类命令。
  // 可以先把它理解成：让这台机器进入“可被远程连接或控制”的模式。
  ...
  if (feature('TEMPLATES') && (args[0] === 'new' || args[0] === 'list' || args[0] === 'reply')) {
    ...
    // 这里不只是 return，而是直接退出程序。
    // 这样能避免界面相关的残留状态让进程退不干净。
    // eslint-disable-next-line custom-rules/no-process-exit
    process.exit(0);
  }

  // 这一段处理 environment-runner。
  // 它是不带交互界面的运行方式。
  ...
  // 这一段处理 self-hosted-runner。
  // 和上面类似，也是一个不带交互界面的运行方式。
  ...
  // 如果用户同时开了 worktree 和 tmux，
  // 就先试着切进对应的 tmux 会话，成功的话就不用继续往下走了。
  ...
  // 有些人会把更新命令写成 --update 或 --upgrade。
  // 这里顺手帮他改成程序真正认识的 `update`。
  if (args.length === 1 && (args[0] === '--update' || args[0] === '--upgrade')) {
    process.argv = [process.argv[0]!, process.argv[1]!, 'update'];
  }

  // `--bare` 表示走更精简的模式，
  // 所以这里提前把对应开关打开，免得后面加载代码时来不及。
